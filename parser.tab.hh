...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YY_ASSERT (tok == 0 || tok == token::CLASS || tok == token::MAIN || tok == token::EXTENDS || tok == token::STRING || tok == token::PUBLIC || tok == token::STATIC || tok == token::VOID || tok == token::RETURN || tok == token::LENGTH || tok == token::IF || tok == token::ELSE || tok == token::WHILE || tok == token::SYS_PRINTLN || tok == token::THIS || tok == token::NEW || tok == token::PLUSOP || tok == token::MINOP || tok == token::MULOP || tok == token::DIVOP || tok == token::ASSIGN_OP || tok == token::AND || tok == token::OR || tok == token::EQ || tok == token::LT || tok == token::GT || tok == token::DOT || tok == token::NOT || tok == token::LHKP || tok == token::RHKP || tok == token::LP || tok == token::RP || tok == token::LMP || tok == token::RMP || tok == token::COMMA || tok == token::SEMICOLON);
      }
#else
      symbol_type (int tok)
        : super_type(token_type (tok))
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      make_MINOP ()
      {
        return symbol_type (token::MINOP);
      }
#else
      static
      symbol_type
      ...
      make_MULOP ()
      {
        return symbol_type (token::MULOP);
      }
#else
      static
      symbol_type
